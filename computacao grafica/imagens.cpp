...
int main(){
   int x1=-20;
   ALLEGRO_BITMAP *terra, *lua, *carro;

   al_init();
   al_init_primitives_addon();
   al_init_image_addon();      // #include <allegro_image.h>
   al_install_keyboard();
   al_install_mouse();
   ...
   terra = al_load_bitmap("terra.png");
   lua = al_load_bitmap("lua.png");
   carro = al_load_bitmap("tesla.png");
   int carroW = al_get_bitmap_width(carro);
   int carroH = al_get_bitmap_height(carro);
   ...
   while (true){
       ...
       if (teclas[3]) { r2.y1-=2; } //W
        ...
        al_draw_bitmap(terra, 0, 100, 0);
        al_draw_scaled_bitmap(lua,0,0,250,250,550,200,150,150,0);
        al_draw_rotated_bitmap(carro,carroW/2, carroH/2, 400, 300, -45.0*3.14159/180.0,ALLEGRO_FLIP_HORIZONTAL + ALLEGRO_FLIP_VERTICAL);
